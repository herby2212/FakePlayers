...
public class PlayerListHandler_1_11_R1 implements PlayerListHandler {
	
	private static HashMap<String, CustomPlayer_1_11_R1> customPlayers = new HashMap<String, CustomPlayer_1_11_R1>();
	private static List<CustomPlayer_1_11_R1> fakedPlayersStorage = new ArrayList<CustomPlayer_1_11_R1>();
	...
	public void setOnlinePlayers(int amount) {
        ...
        try {
            ...
            for(int i = 0; i < amount; i++) {
            	CustomPlayer_1_11_R1 cp = new CustomPlayer_1_11_R1((PlayerInteractManager) playerInteractManagerConstructor.newInstance(nms_world), "", UUID.fromString("3e5cf803-1183-43f5-a53d-ea53c61b6274"));
    ...
    public void removeOnlinePlayers() {
    	...
    	for(CustomPlayer_1_11_R1 cp : fakedPlayersStorage) {
    		list.remove(cp);
    	}
    	Main.instance.fakePlayersCount = amount;
    }
    ...
    public void removeCustomOnlinePlayers() {
    	for(CustomPlayer_1_11_R1 cp : customPlayers.values()) {
    		list.remove(cp);
    	}
    }
    
    public void addCustomOnlinePlayer(String name) {
		CustomPlayer_1_11_R1 customPlayer = new CustomPlayer_1_11_R1(new PlayerInteractManager(((CraftWorld) Bukkit.getServer().getWorld(Main.instance.defaultWorldName)).getHandle()), name, RandomUUID.randomUUID());		
    ...
    public void removeCustomOnlinePlayer(String name) {
		if(customPlayers.containsKey(name)) {
			CustomPlayer_1_11_R1 cp = customPlayers.get(name);
			list.remove(cp);
